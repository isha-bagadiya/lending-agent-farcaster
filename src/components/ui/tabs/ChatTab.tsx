  ...
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"} mb-6`}>
      <div
        ...
        } items-end space-x-2 ${isUser ? "space-x-reverse" : ""}`}
      >
        {/* Avatar */}
        <div
...
export function ChatTab() {
  ...
  useEffect(() => {
    if (isTxConfirmed && txHash && pendingTransaction) {
      ...
      addMessage(
        "system",
        `🎉 Transaction confirmed! ${summary} completed successfully.`
      );
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      if (!isConnected) {
        addMessage(
          "system",
          "🔗 Please connect your MetaMask wallet first by going to the Wallet tab."
        );
        return;
      }
      ...
      if (chainId !== arbitrum.id) {
        ...
        try {
          ...
          addMessage(
            "system",
            "❌ Failed to switch to Arbitrum network. Please switch manually in your wallet."
          );
          return;
        }
      }
          ...
          if (artifact?.parts?.[0]?.data) {
            ...
            if (artifactData.txPreview && artifactData.txPlan) {
              ...
              const formattedTransactionData =
                formatTransactionData(rawTransactionData);
            ...
            // Check if this is positions/balance data
              ...
              artifactData.positions &&
              Array.isArray(artifactData.positions) &&
              artifactData.positions.length > 0
            ) {
...
• Liquidation Threshold: ${formatPercentage(
                positionData.currentLiquidationThreshold
              )}
• Health Factor: ${positionData.healthFactor}

${
  parseFloat(positionData.healthFactor) > 2
    ? "✅ Your position is healthy!"
    : parseFloat(positionData.healthFactor) > 1.2
    ? "⚠️ Monitor your position closely"
    : "🚨 Risk of liquidation!"
}`;
          ...
          addMessage(
            "assistant",
            "🤔 Sorry, I couldn't process your request. Could you please try rephrasing it or be more specific about what you'd like to do?"
          );
        }
      } catch (error) {
  ...
  const handleTransactionApprove = useCallback(async () => {
    if (
      !pendingTransaction ||
      currentTxIndex >= pendingTransaction.txPlan.length
    ) {
      return;
    }
    ...
    try {
      ...
      addMessage(
        "system",
        `📝 Sending ${stepType} transaction (${currentTxIndex + 1}/${
          pendingTransaction.txPlan.length
        ...
        },
        {
          onSuccess: (hash) => {
            ...
            if (currentTxIndex + 1 < pendingTransaction.txPlan.length) {
              ...
              const nextStepType =
                currentTxIndex + 1 === 0 ? "Approval" : "Supply";
              addMessage(
                "system",
                `✅ ${stepType} transaction sent!\n📤 Hash: ${truncateAddress(
                  hash
      ...
      addMessage(
        "system",
        "❌ Transaction failed. Please try again or check your wallet connection."
      );
      setPendingTransaction(null);
      setCurrentTxIndex(0);
    }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-200px)] relative overflow-hidden">
      ...
      {(!isConnected || chainId !== arbitrum.id) && (
        <div
          className={`mx-4 my-auto mb-auto p-4 rounded-2xl border backdrop-blur-sm ${
            !isConnected
            ...
            <div className="flex justify-center">
              <div
                className={`w-12 h-12 rounded-xl flex items-center justify-center shadow-md ${
                  !isConnected
                    ? "bg-gradient-to-br from-[#200d36] to-black"
                    : "bg-gradient-to-br from-[#200d36] to-black"
                }`}
              >
                {!isConnected ? (
            ...
            <div className="space-y-2">
              {!isConnected ? (
                <>
                  ...
                  <p className="text-sm text-[#200d36] max-w-sm mx-auto">
                    Connect your MetaMask wallet to start your DeFi journey on
                    Arbitrum.
                  </p>
                </>
              ) : (
                <>
                  ...
                  <p className="text-sm text-[#200d36] max-w-sm mx-auto">
                    Switch to Arbitrum network in your wallet to use DeFi
                    features.
                  </p>
                </>
              )}
      ...
      {isConnected && chainId === arbitrum.id && (
        <>
          <div
            ...
            style={{
              marginTop:
                !isConnected || chainId !== arbitrum.id ? "20px" : "0px",
            }}
          >

            {/* Messages Scroll Area */}
            <div
              className="h-full overflow-y-auto px-4 py-6 pb-24 space-y-6"
              style={{
                scrollBehavior: "smooth",
                paddingTop:
                  !isConnected || chainId !== arbitrum.id ? "10px" : "24px",
              }}
            >
              {/* Welcome Message for Empty State */}
              {messages.length <= 1 && (
                <div className="text-center py-8 ">
                  <div className="w-16 h-16 mx-auto mb-4 bg-purple-100 rounded-full flex items-center justify-center shadow-lg">
                    <svg
              ...
              {isTxError && (
                <div className="flex justify-center mb-6 animate-in slide-in-from-top duration-300">
                  <div className="bg-gradient-to-br from-red-50 via-red-50 to-red-100 dark:from-red-900/20 dark:via-red-900/25 dark:to-red-900/30 border border-red-200 dark:border-red-700/50 rounded-2xl p-6 max-w-2xl w-full shadow-xl">
                    ...
                    <div className="ml-11">{renderError(txError)}</div>
                  </div>
                </div>
              )}
            ...
            >
              <div className="flex-1 relative">
                <input
                  ...
                  disabled={
                    isLoading || !isConnected || chainId !== arbitrum.id
                  }
                  className="w-full px-2 py-3 pr-6 border-2 border-gray-300 rounded-xl bg-white  text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-none shadow-md transition-all duration-200 text-base font-medium disabled:opacity-50 relative z-10"
                  autoComplete="off"
                  style={{
                    pointerEvents: "auto",
                    cursor:
